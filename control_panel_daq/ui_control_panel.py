...
class ControlPanelWindow(QMainWindow):
    ...
    def __init__(self, xbee_manager_instance, data_processor_instance, parent=None):
        ...
        # REVERTED: Font sizes back to original
    ...
    def _init_ui(self):
        ...
        # Removed: right_column_layout.addStretch(1) # Allow top_group_box to take space
    ...
    def _create_sensor_display_and_board_status_section(self, parent_layout):
        ...
        # REVERTED: Spacing back to original
        self.pt_sensors_display_layout.setSpacing(10)
        pt_column_vbox_layout.addLayout(self.pt_sensors_display_layout)
        # MODIFIED: Removed internal stretch to make group box height tighter to content
        ...
        top_group_layout.addWidget(scroll_area, 1) # MODIFIED: Added stretch factor 1 (original was 1)
        ...
        parent_layout.addWidget(top_group_box, 1) # MODIFIED: Added stretch factor 1 (original was 1)
    ...
    def _populate_sensor_columns_dynamically(self):
        ...
        for sensor_conf in sorted_components:
            ...
            if comp_type_for_ui_key is None:
                if comp_name.startswith("PT"):
                    ...
                    comp_type_for_ui_key = "LoadCell"
                elif comp_name.startswith("AM"):
                    comp_type_for_ui_key = "Accelerometer"
                elif comp_name.startswith("GM"):
                    comp_type_for_ui_key = "Gyrometer"
                elif comp_name.startswith("BM"):
                    comp_type_for_ui_key = "Barometer"

            if comp_type_for_ui_key not in ["PressureTransducer", "Thermocouple", "LoadCell", "Heater", "Accelerometer", "Gyrometer", "Barometer"]:
                continue
            ...
            name_label.setTextFormat(Qt.RichText)
            name_label.setFont(self.SENSOR_NAME_FONT) # Original font size
            name_label.setWordWrap(True)

            value_label = QLabel("N/A")
            # REVERTED: Minimum width back to original
            ...
            value_label.setFont(self.SENSOR_VALUE_FONT) # Original font size
        ...
        if not pt_added:
            ...
            placeholder_label.setFont(self.SENSOR_NAME_FONT) # Original font size
    ...
    def _create_general_commands_group(self, parent_layout):
        ...
        general_commands_layout.addWidget(self.tare_pt_button, row, 0) 
        ...
        general_commands_layout.addWidget(self.tare_lc_button, row, 1) 
    ...
    def _update_transmit_status_display(self, status_info):
        ...
        if address_64bit in self._radio_ui_elements:
            ...
            elif any(err in status_text.lower() for err in ["fail", "error", "timeout", "n/a", "no ack", "invalid", "skipped"]): 
    ...
    def _update_radio_status_display(self, radio_info):
        ...
        if addr in self._radio_ui_elements:
            ...
            if radio_info.get('is_connection_lost', False): 
    ...
    def _update_board_general_connectivity(self, board_id_8bit: int, board_name: str, timestamp: float):
        ...
        if board_id_to_update is None: 
            board_id_to_update = board_id_8bit
    ...
    def _update_sensor_display(self, name, value_str, unit, board_name, component_type_name):
        ...
        if key in self._ui_elements:
            ...
            value_label.setFont(self.SENSOR_VALUE_FONT) # Original font
        ...
        for key, widget in self._ui_elements.items():
            if isinstance(widget, QLabel):
                if "_Servo_status" in key:
                    ...
                    widget.setFont(self.SENSOR_VALUE_FONT) # Original font
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self.data_processor, 'close_labjack'): 
...
if __name__ == '__main__':
    ...
    class MockXBeeManager(QObject):
        ...
        def _simulate_board_status_response(self):
             board_ids = [bid for bid, binfo in config.BOARD_INFO_LOOKUP_TABLE.items() if binfo.get("type") not in ["ServoBoard"]] 
             if board_ids:
    ...
    class MockDataProcessor(QObject):
        ...
        def simulate_periodic_updates(self):
            ...
            if config.LABJACK_ENABLED and hasattr(config, 'LABJACK_SUMMED_LC_NAME') and hasattr(config, 'LABJACK_LOADCELL_UNIT'):
                ...
                self._mock_last_raw_values[f"{lc_name}_LoadCell"] = raw_weight 
            ...
            if config.ALL_COMPONENT_CONFIGS and random.random() < 0.7:
                ...
                if is_labjack_sensor_item or is_not_can_source_for_this_loop:
                    ...
                    if config_defined_type == 'PressureTransducer' or (not config_defined_type and comp_name.startswith("PT")):
                        ...
                        raw_float_val = random.uniform(18.0, 55.0) 
