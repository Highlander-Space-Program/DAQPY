...
class ControlPanelWindow(QMainWindow):
    ...
    def __init__(self, xbee_manager_instance, data_processor_instance, parent=None):
        ...
        # BOARD_STATUS_CHECK_TIMER_MS can be made more frequent if needed, e.g., 1000ms
    ...
    def _init_ui(self):
        ...
        # Spacing between the main groups in the top bar.
        # This will be the space between "XBee Connection" and "UI Settings",
        # and between "UI Settings" and the "Radio Status" area if no stretch is between them.
        top_bar_layout.setSpacing(15)

        # XBee Connection Group
        conn_group = QGroupBox("XBee Connection")
        conn_layout = QHBoxLayout(conn_group)
        conn_layout.setSpacing(5) # Internal spacing
        conn_layout.setContentsMargins(5, 5, 5, 5) # Internal padding
        ...
        # conn_layout.addStretch(1) # Optional: if you want XBee contents pushed left
        top_bar_layout.addWidget(conn_group) # Added with default stretch 0

        # UI Settings Group - will now be right next to conn_group (respecting top_bar_layout.setSpacing)
        ...
        # ui_rate_layout.addStretch(1) # Optional: pushes content left within this group
        top_bar_layout.addWidget(ui_rate_group) # Added with default stretch 0

        # Add ONE stretchable space AFTER ui_rate_group to push everything before it to the left.
        # This stretch will now take up all remaining space to the right of ui_rate_group,
        # before radio_status_widget.
        top_bar_layout.addStretch(1)

        # Radio Status Area (No GroupBox)
        radio_status_widget = QWidget()
        radio_status_layout = QHBoxLayout(radio_status_widget)
        # Add 10px padding around the radio grid and button
        radio_status_layout.setContentsMargins(20, 20, 20, 20) # Padding for radio stuff
        radio_status_layout.setSpacing(6) # Spacing between radio grid and radio button
        self._create_radio_status_elements(radio_status_layout)
        # If you want contents within radio_status_widget (grid+button) pushed left
        # DESPITE the new external padding, you could add a stretch here:
        # radio_status_layout.addStretch(1)
        top_bar_layout.addWidget(radio_status_widget, 0) # Stretch 0 for preferred size
    ...
    def _initialize_board_connectivity_info(self):
        ...
        # This already includes CASEY (Pad Controller) if it's in BOARD_INFO_LOOKUP_TABLE
    ...
    def _create_radio_status_elements(self, parent_layout):
        ...
        for name, addr_hex in config.XBEE_TARGET_RADIO_CONFIG:
            ...
            last_tx_desc_label.setMinimumWidth(300) # TRY REDUCING (e.g. to 100-120)
            ...
            last_tx_status_label.setMinimumWidth(100) # TRY REDUCING (e.g. to 50-60)
    ...
    def _create_sensor_display_and_board_status_section(self, parent_layout):
        ...
        # --- Board Connections (Non-Servo Boards + PAD CONTROLLER) ---
        sensor_boards_status_group = QGroupBox("Other Board Connection Status") # Renamed for clarity
        ...
        max_status_cols = 3 # Adjust as needed

        # Display all boards not shown in the servo panel
        board_ids_for_status_panel = sorted([
            bid for bid, binfo in self._board_connectivity_info.items()
            if binfo.get("type") != "ServoBoard" # Shows SensorBoard, ControllerBoard, etc.
        ])
        ...
        if not board_ids_for_status_panel:
            ...
            for board_id_hex in board_ids_for_status_panel:
                ...
                self._sensor_panel_board_status_labels[board_id_hex] = status_indicator # Store for updates specific to this panel
                if status_indicator not in board_conn_info["ui_labels"]: # Ensure it's in the general list too
                ...
                if status_col >= max_status_cols:
                    status_col = 0
                    status_row += 1
            if status_row == 0 and status_col < max_status_cols : # If only a few items, prevent excessive vertical stretch
            ...
            elif status_row > 0 : # Add spacer if multiple rows used, to push content up
                 sensor_boards_status_layout.addItem(QSpacerItem(20,10, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Expanding), status_row +1, 0, 1, max_status_cols*2)
    ...
    def _populate_sensor_columns_dynamically(self):
        ...
        for sensor_conf in sorted_components:
            ...
            # --- START MODIFICATION TO TYPE DETERMINATION ---
            comp_type_for_ui_key = None # This will be "PressureTransducer", "Thermocouple", or "LoadCell" etc.
            is_pt_sensor = False
            target_layout = self.other_sensors_display_layout # Default

            # Try to determine type from 'type' field in config first
            # This type string (e.g., "LoadCell") must match what data_processor.py emits in the signal
            ...
            if config_defined_type == 'PressureTransducer':
                ...
                # target_layout is already self.other_sensors_display_layout
            elif config_defined_type == 'LoadCell': # Catches LabJack LoadCell if config has type: "LoadCell"
                comp_type_for_ui_key = 'LoadCell'
                # target_layout is already self.other_sensors_display_layout
            elif config_defined_type == 'Heater': # If you display heaters in the sensor panel
                 comp_type_for_ui_key = 'Heater'
                 # target_layout is already self.other_sensors_display_layout

            # Fallback to prefix if type not in config or not one of the recognized sensor types above
            if comp_type_for_ui_key is None:
                if comp_name.startswith("PT"):
                    ...
                    comp_type_for_ui_key = "Thermocouple"
                elif comp_name.startswith("LC"): # Catches conventionally named LoadCells
                    comp_type_for_ui_key = "LoadCell"

            # If component type for UI key is still not determined or not a displayable sensor type, skip.
            # Ensure this list includes all types handled by the ui_update_sensor signal for this panel.
            if comp_type_for_ui_key not in ["PressureTransducer", "Thermocouple", "LoadCell", "Heater"]:
                # app_logger.debug(f"Skipping component '{comp_name}' for sensor panel: Unrecognized or unhandled type for display.")
                continue
            # --- END MODIFICATION TO TYPE DETERMINATION ---

            parent_board_id = sensor_conf.get('parent_board_id_hex')
            # Use parent_board_name directly from config if provided (useful for LabJack)
            parent_board_name = sensor_conf.get('parent_board_name')
            if parent_board_name is None: # Fallback for CAN-based components
            ...
            # Store value label for updates using the derived component type for the key
        ...
        if not other_added:
            placeholder_label = QLabel("<i>No TC/LC/Other sensors configured.</i>") # Adjusted placeholder
            placeholder_label.setFont(self.SENSOR_NAME_FONT)
    ...
    def _check_board_timeouts(self):
        ...
        default_timeout_s = config.BOARD_ACK_TIMEOUT_MS / 1000.0  # Default 10 seconds
        pad_controller_timeout_s = config.PAD_CONTROLLER_ACK_TIMEOUT_MS / 1000.0 # 5 seconds

        pad_controller_board_id = config.get_board_id_by_name("CASEY") # Assuming CASEY is the PAD_CONTROLLER
        ...
        for board_id_hex, info in self._board_connectivity_info.items():
            ...
            # Any message from a board updates its 'last_seen' via _update_board_general_connectivity.
            # So, this check correctly determines if it has been too long since *any* message.
...
if __name__ == '__main__':
    import random # Ensure random is imported for mocks
    ...
    class MockXBeeManager(QObject):
        ...
        def _simulate_board_status_response(self):
             ...
             board_ids = [bid for bid, binfo in config.BOARD_INFO_LOOKUP_TABLE.items() if binfo.get("type") not in ["ServoBoard", "ControllerBoard"]]
    ...
    class MockDataProcessor(QObject):
        ...
        _ui_update_timer = QTimer() # Renamed for clarity if it was different before

        def __init__(self):
            super().__init__()
            self._ui_update_timer = QTimer(self) # Main timer for all mock updates
        ...
        def set_ui_update_frequency(self, hz):
            ...
            if interval_ms > 0:
                ...
                if not self._ui_update_timer.isActive(): # Start if not already running
                    self._ui_update_timer.start()
            else: # hz is 0 or less, stop the timer
        ...
        def simulate_periodic_updates(self):
            # This method is now called at the rate set by set_ui_update_frequency
            ...
            # --- Simulate LabJack Load Cell Data (at main UI update frequency) ---
            if config.LABJACK_ENABLED and hasattr(config, 'LABJACK_SUMMED_LC_NAME') and hasattr(config, 'LABJACK_LOADCELL_UNIT'):
                ...
                lc_board_name = "LabJack DAQ" # Default
                if lj_lc_conf:
                     lc_board_name = lj_lc_conf.get("parent_board_name", "LabJack DAQ")

                simulated_base_weight = random.uniform(200, 800) # lbs
                noise = random.uniform(-20, 20) # lbs
                total_summed_weight = simulated_base_weight + noise + 72.0 # Adding the 72 offset
                
                value_str = f"{total_summed_weight:.1f}"
                self.ui_update_sensor.emit(lc_name, value_str, lc_unit, lc_board_name, "LoadCell")
            # --- End LabJack Simulation ---


            # Simulate OTHER sensor data (PT, TC, CAN LC)
            # This loop will now iterate and simulate CAN-based sensors.
            if config.ALL_COMPONENT_CONFIGS and random.random() < 0.7: # Probability for CAN sensor update
                sensor_conf = random.choice(config.ALL_COMPONENT_CONFIGS)
                comp_name = sensor_conf['name']
                
                # Explicitly skip if this is the LabJack sensor config item,
                # or if its source_type indicates it's not a CAN sensor this loop should handle.
                ...
                if is_labjack_sensor_item or is_not_can_source_for_this_loop:
                    pass # Skip: LabJack handled above, or not a CAN sensor for this generic loop
                else:
                    # This is a CAN-based sensor (or other sensor not handled elsewhere)
                    comp_type_name = "Unknown" 
                    val_str, unit_str = "N/A", ""
                    config_defined_type = sensor_conf.get('type')

                    # Determine type based on config or name prefix (simplified for mock)
                    if config_defined_type == 'PressureTransducer' or (not config_defined_type and comp_name.startswith("PT")):
                        comp_type_name = "PressureTransducer"; val_str, unit_str = f"{random.uniform(-5.0, 1550.0):.1f}", sensor_conf.get("unit", "PSI")
                    elif config_defined_type == 'Thermocouple' or (not config_defined_type and comp_name.startswith("TC")):
                        comp_type_name = "Thermocouple"; val_str, unit_str = f"{random.uniform(18.0, 55.0):.1f}", sensor_conf.get("unit", "°C")
                    elif config_defined_type == 'LoadCell': # Catches CAN LoadCells
                        comp_type_name = "LoadCell"; val_str, unit_str = f"{random.uniform(-100.0, 5100.0):.1f}", sensor_conf.get("unit", "lbf")
                    
                    if comp_type_name != "Unknown":
                        parent_board_id = sensor_conf.get("parent_board_id_hex")
                        parent_board_name = "Unknown Board" # Default
                        if parent_board_id is not None: # Check if it's a CAN sensor with a board ID
    ...
    # Start the mock data simulation timer using the default rate from config
    # The user can change this via the UI, and set_ui_update_frequency will be called.
